      ...
      <section className={styles.lastestEpisodes}>
        ...
        <ul>
          {lastestEpisodes.map((episode, index)=>{
            return (
              <li key={episode.id}>
                <Image width={192} height={192} src={episode.thumbnail} alt={episode.description} objectFit="cover" />                
...
export const getStaticProps:GetStaticProps =  async () =>{
  ...
  const episodes = data.map(episode =>{
    return{
      ...
      publishedAt: format(parseISO(episode.published_at), 'd MMM yy', {locale:
        ptBR
      }),      
